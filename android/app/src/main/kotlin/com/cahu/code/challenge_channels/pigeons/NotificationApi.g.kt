...
)
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): NotificationData {
      ...
      return NotificationData(notificationId, title, body, payload)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    if (other !is NotificationData) {
      return false
    }
    if (this === other) {
      return true
    }
    return NotificationApiPigeonUtils.deepEquals(toList(), other.toList())  }
...
interface NotificationApi {
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: NotificationApi?, messageChannelSuffix: String = "") {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.pigeon_notification_package.NotificationApi.showNotification$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val dataArg = args[0] as NotificationData
            val wrapped: List<Any?> = try {
              api.showNotification(dataArg)
              listOf(null)
